...
df<-EXO_PME_raw %>% 
  ...
  select(Timestamp, value, temp_corr, Temp_C)

temp <- fun_temp(df)

df <- rbind(temp, df) %>% 
  mutate("temp_corr" = as.factor(temp_corr))
...
#run function
drift<-fun_drift(df, start, end, cleaned)


#Replace values in master df
df<-bind_rows(df, drift) %>% 
  mutate("drift_corr" = as.factor(drift_corr))
...
df <- EXO_PME_raw %>% 
  ...
  select(value, Timestamp, `Site Name`) %>% 
...
#run the function
anomaly <- fun_anomalous(df, min, max)
