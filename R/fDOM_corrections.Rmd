...
DK_SW_EXO_20200919_raw <- read_csv(paste0(data_dir,"/DK_SW_fDOM_20200919_no_head.csv")) %>% 
...
DK_SW_EXO_20201026_raw <- read_csv(paste0(data_dir,"/DK_SW_fDOM_20201026_no_head.csv")) %>% 
  rename("Date" = `Date (MM/DD/YYYY)`, 
         "Time" = `Time (HH:mm:ss)`, 
         "fDOM_QSU" = `fDOM QSU`, 
         "Turb_FNU" = `Turbidity FNU`,
...
DK_SW_PME_20200919_trimmed <- read_delim(paste0(data_dir,"/DK_SW_PME_20200906_20200919.txt"),
                             delim = ",",
                             skip = 9, 
                             col_names = FALSE) %>% 
  select(c(X3, X5)) %>% 
...
DK_SW_PME_20201026_trimmed <- read_delim(paste0(data_dir,"/DK_SW_PME_20200919_20201026.txt"),
                             delim = ",",
                             skip = 9, 
                             col_names = FALSE) %>% 
  select(c(X3, X5)) %>% 
...
#Merge the PME files
PME_ready <- rbind(DK_SW_PME_20200919_trimmed, DK_SW_PME_20201026_trimmed, ND_SW_PME_20200919_trimmed, ND_SW_PME_20201026_trimmed) %>% 
...
## 5. Using the Anomaly_funtion to remove weird values. 
...
df <- EXO_PME_raw %>% 
  filter(`Site Name` == "DK") %>% 
  mutate("value" = as.numeric(QSU_temp_corrected), 
         "Timestamp" = Date_Time_EST) %>% 
  select(value, Timestamp) %>% 
  mutate(anomalous_corr = 0)
...
#run the function
temp <- fun_anomalous(df, min, max)

df <- bind_rows(temp, df) %>% 
  mutate("anomalous_corr" = as.factor(anomalous_corr))

ggplot(data = df,
       mapping = aes(x = Timestamp,
                     y = value,
                     color = anomalous_corr)) +
  geom_line()
