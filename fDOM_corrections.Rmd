...
Example data sets used: ND-SW from 9/6 to 10/29 and DK SW from 
---
...
setwd("C:/Users/James Maze/Desktop/Delmarva/Github/fDOM_QAQC")
...
fDOM_Temp_corr <- EXO_PME_raw_timestamped %>% 
...
DK_fDOM_Temp_compare <- fDOM_Temp_corr %>% 
  filter(`Site Name` == "DK") %>% 
  select(fDOM_QSU, QSU_temp_corrected, Date_Time_EST) 

#Change to an xts
DK_fDOM_Temp_compare <- DK_fDOM_Temp_compare %>% 
  xts(order.by = DK_fDOM_Temp_compare$Date_Time_EST)

#This code makes the graph
(dygraph(DK_fDOM_Temp_compare, 
...
#This code just manipulates the data so that it can be plotted with the dygraph package
ND_fDOM_Temp_compare <- fDOM_Temp_corr %>% 
  filter(`Site Name` == "ND") %>% 
  select(fDOM_QSU, QSU_temp_corrected, Date_Time_EST) 

#Change to an xts
ND_fDOM_Temp_compare <- ND_fDOM_Temp_compare %>% 
  xts(order.by = ND_fDOM_Temp_compare$Date_Time_EST)

#This code makes the graph
(dygraph(ND_fDOM_Temp_compare, 
